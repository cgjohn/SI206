...
## There are tests for each problem; you should also follow the instructions for things to do in the code that cannot 
# be tested with unit tests, e.g. using certain structures.
...
def personal_map(func, li):
    new_li = []
    for i in li:
        new_li.append(func(i))
    return new_li
...
## [PROBLEM 5]
print("\n\n***** Problem 5 *****")
# Use a list comprehension to create a list of Student instances out of the student_tups 
#list you just created in Problem 2, and save that list in a variable called programmers. 
#You should make sure you pass these tests before continuing, as you'll need this list for 
#problems later on!
...
# Save the list of tuples in a variable called names_and_productivities. To do this, you should use a list 
# comprehension (you may also use the zip function, and you may use any variables you have already created).
...
# Use a list comprehension to generate a LIST of just the names of those Student instances whose name is 
# longer than their seniority (i.e., ["Albert", "Mai", "Dinesh", "Euijin"]). Assign it to a variable called 
...
## Define a function called readfiles, which accepts a list of filenames as input
#  and yields each line in each of the file with that name, assuming those files exist in the same directory as this program.

## Define a generator called len_check which accepts a generator of file lines and 
# returns a generator object of all the lines it's accepted whose length is longer than 40 characters.

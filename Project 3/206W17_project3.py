## SI 206 Winter 2017 Project 3 Building on HW6, HW7 (and some previous
## material!)

## NOTE: There are tests for this project, but the tests are NOT exhaustive --
## you should pass them, but ONLY passing them is not necessarily sufficient to
## get 100% on the project. The caching must work correctly, the
## queries/manipulations must follow the instructions and work properly. You can
## ensure they do by testing just the way we always do in class -- try stuff
## out, print stuff out, use the SQLite DB Browser and see if it looks ok!

## You may turn the project in late as a comment to the project assignment at a
## deduction of 10 percent of the grade per day late. This is SEPARATE from the
## late assignment submissions available for your HW.
...
import requests
import collections
import sqlite3 # same deal as always... import json import sqlite3
...
## (This may sound familiar...) We have provided a CACHE_FNAME variable for you
## for the cache file name, but you must write the rest of the code in this
## file.

CACHE_FNAME = "SI206_project3_cache.json" # Put the rest of your caching setup here:
...
# Write an invocation to the function for the "umich" user timeline and save the
# result in a variable called umich_tweets:
...
# You will be creating a database file: project3_tweets.db Note that running the
# tests will actually create this file for you, but will not do anything else to
# it like create any tables; you should still start it in exactly the same way
# as if the tests did not do that!  The database file should have 2 tables, and
# each should have the following columns...

# table Tweets, with columns: - tweet_id (containing the string id belonging to
# the Tweet itself, from the data you got from Twitter -- note the id_str
# attribute) -- this column should be the PRIMARY KEY of this table - text
# (containing the text of the Tweet) - user_id (an ID string, referencing the
# Users table, see below) - time_posted (the time at which the tweet was
# created) - retweets (containing the integer representing the number of times
...
# table Users, with columns: - user_id (containing the string id belonging to
# the user, from twitter data -- note the id_str attribute) -- this column
# should be the PRIMARY KEY of this table - screen_name (containing the screen
# name of the user on Twitter) - num_favs (containing the number of tweets that
# user has favorited) - description (text containing the description of that
# user on Twitter, e.g. "Lecturer IV at UMSI focusing on programming" or "I
# tweet about a lot of things" or "Software engineer, librarian, lover of
# dogs..." -- whatever it is. OK if an empty string)

## You should load into the Users table: 
# The umich user, and all of the data about users that are mentioned in the umich timeline.  
# NOTE: For example, if the user with the "TedXUM" screen name is mentioned in the umich timeline, that
# Twitter user's info should be in the Users table, etc.
...
## want about the user. HINT #2: You may want to go back to a structure we used
## in class this week to ensure that you reference the user correctly in each
## Tweet record. HINT #3: The users mentioned in each tweet are included in the
## tweet dictionary -- you don't need to do any manipulation of the Tweet text
## to find out which they are! Do some nested data investigation on a dictionary
## that represents 1 tweet to see it!
...
# All of the following sub-tasks require writing SQL statements and executing
# them using Python.

# Make a query to select all of the records in the Users database. Save the list
# of tuples in a variable called users_info.
cur.execute('SELECT * FROM Users')
users_info = cur.fetchall()

# Make a query to select all of the user screen names from the database. Save a
# resulting list of strings (NOT tuples, the strings inside them!) in the
# variable screen_names. HINT: a list comprehension will make this easier to
# complete!
cur.execute('SELECT screen_name FROM Users')
users_and_sns = cur.fetchall()
screen_names = [str(sn) for sn in users_and_sns]
...
# Make a query to select all of the tweets (full rows of tweet information) that
# have been retweeted more than 25 times. Save the result (a list of tuples, or
# an empty list) in a variable called more_than_25_rts.
cur.execute('SELECT * FROM Tweets WHERE retweets > 5')
more_than_25_rts = cur.fetchall()

# Make a query to select all the descriptions (descriptions only) of the users
# who have favorited more than 25 tweets. Access all those strings, and save
# them in a variable called descriptions_fav_users, which should ultimately be a
# list of strings.
cur.execute('SELECT description FROM Users WHERE num_favs > 5')
users_descr = cur.fetchall()
descriptions_fav_users = [str(de) for de in users_descr]


# Make a query using an INNER JOIN to get a list of tuples with 2 elements in
...
## Use a set comprehension to get a set of all words (combinations of characters
## separated by whitespace) among the descriptions in the descriptions_fav_users
## list. Save the resulting set in a variable called description_words.
...
## Use a Counter in the collections library to find the most common character
## among all of the descriptions in the descriptions_fav_users list. Save that
## most common character in a variable called most_common_char. Break any tie
## alphabetically (but using a Counter will do a lot of work for you...).
...
# Write code to create a dictionary whose keys are
## Twitter screen names and whose associated values are lists of tweet texts that
## that user posted. You may need to make additional queries to your database! To
## do this, you can use, and must use at least one of: the DefaultDict container in
...
### IMPORTANT: MAKE SURE TO CLOSE YOUR DATABASE CONNECTION AT THE END OF THE
### FILE HERE SO YOU DO NOT LOCK YOUR DATABASE (it's fixable, but it's a pain).
### ###
conn.close()

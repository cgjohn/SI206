...
#instructions/hints/examples, and at the end, tests. See the PDF of instructions 
#for more detail. 
...
# Set up to be able grab stuff from twitter with your authentication using 
#Tweepy methods, and return it in a JSON format 
...
#### Below if the caching function from HW 5 ####
...
stri = "http://www.google.com is a great site should "


def find_urls(inp):
	matches = []
	matches = re.findall('http\S+', inp)
	return matches
...
## INPUT: N/A. No input.
## The function should check if there is any cached data for the UMSI 
#directory in your cached file -- if so, return it, and if not, the function should access each page
# of the directory, get the HTML associated with it, append that HTML string to a list. The function 
#should cache (save) that list when it is accumulated.
...
## INPUT: Any string
## Behavior: See instructions. Should search for the input string on twitter and get results. 
#Should check for cached data, use it if possible, and if not, cache the data retrieved.
...
## PART 3 (b) - Write one line of code to invoke the get_five_tweets function with the phrase 
#"University of Michigan" and save the result in a variable five_tweets.
...
class PartThree(unittest.TestCase):
	...
	def test_tweet_urls_found(self):
		self.assertEqual(set([x[:4] for x in tweet_urls_found]),set(["http"]),
			"Testing that each element in tweet_urls_found list is a URL begin. with HTTP")

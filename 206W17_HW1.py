...
## The function name_counts takes as input a list of strings. It should return a list of tuples,
# where each tuple contains a UNIQUE string from the list and the count of that string's occurrences in the list. 
## The list that is returned from your function should be in decreasing order (beginning with a tuple of the word 
#that occurs the most times with its count, and ending with a tuple of the word that occurs the fewest times). 
#In case of ties of counts, break the tie by string value in increasing order, e.g. B comes before C.
...
## Complete the definition of the function build_acronym. This function should accept a list of strings as input. 
...
## Below we've provided a Python class definition representing a house. 
##Add a method to the class called determine_size that returns a string that describes the house's size. 
##If a house's color is "blue", its size should be "big". If a house's color is "red", 
##its size should be "small". Otherwise, its size should be "medium".

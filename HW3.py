...
## find strings of the form: <count> <word>  e.g. 101 Dalmations, inside it, and should return either the LAST 
## correctly matching number-word combo in the string as a tuple, e.g. ('13', "pineapples"), 
## if there are any such sub-strings, or None, if there are not.
## The number in the <count> should be one or more digits only. The word should be made up of an
## optional non-alphabetic character followed by any number of alphabetic characters, upper or lower case.
...
##[0-9]+\s\S*[A-z] <-- Regex Expression
sent = "101 dalmations!"

def parse_counted_words(sent):
    matches = []
    matches = re.findall('[0-9]+\s\S*[A-z]', sent)
    #print(matches)
    if not matches:
        return None
    else:
        words = matches[len(matches) - 1]
        wordsLi = words.split()
        tup = (wordsLi[0], wordsLi[1])
        return tup


## PART 2: 200 points

## We have provided a text file computer_paths.txt. It's not incredibly long -- 
## you can scan through it, but do NOT hard code your answers! Each line contains 1 filesystem path.

## (a) Write Python code to determine how many of these paths identify FILES, 
## not directories. Save that number in the variable file_paths_num.

## (b) Write Python code to determine how many of these paths are FULL paths, 
## not relative paths. Save that number in the variable full_paths_num.

## (c) Write Python code to determine how many of these paths describe a 
## Python file saved inside a folder called SI206. Save that number in the variable python_course_paths.

#SI206[/]\S*[py] <-- regex for part 3
...
# \d\.x|\d\.d <-- regex for that part

# f = open("computer_paths.txt", "r" )
# print(f.read())

with open('computer_paths.txt') as f:
    content = f.readlines()
